...
export function scheduleReview(previous: Review, rating: Rating, now = new Date()): Review {
  ...
  if (rating === 'again') {
    ...
    interval = previous.repetitions === 0 ? 1
      : previous.repetitions === 1 && previous.interval <= 1 ? 3
      : Math.max(1, Math.round(previous.interval * ease));
  } else {
    interval = previous.repetitions === 0 ? 4 : Math.max(4, Math.round(previous.interval * ease * 1.3));
    ease = Math.min(3.5, ease + 0.15);
  }
  interval = Math.min(interval, 3650);
  return { dueAt: new Date(now.getTime() + interval * DAY).toISOString(), interval, ease: Math.round(ease * 100) / 100, repetitions, lastReviewedAt: now.toISOString() };
}
