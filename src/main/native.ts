...
export class NativeService {
  ...
  ready(): Promise<void> {
    ...
    if (process.platform !== 'win32')
      return Promise.reject(new Error('全局选区功能目前仅支持 Windows。'));
    ...
    this.starting = new Promise<void>((resolve, reject) => {
      ...
      const child = spawn(
        ...
        [
          ...
          'Bypass',
          '-File',
          join(this.resourcePath, 'windows-helper.ps1'),
        ],
      ...
      this.lines.on('line', (line: string) => {
        ...
        if (message.ok === true) request.resolve(message.result);
          ...
          request.reject(
            new Error(typeof message.error === 'string' ? message.error : 'Windows 选区操作失败。'),
          );
      });
      ...
      child.once('error', () =>
        ended(new Error('无法启动 Windows 选区服务，请确认 Windows PowerShell 可用。')),
      );
  ...
  async replace(
    ...
    if (!capture.canReplace)
      return {
        ok: false,
        message: capture.replacementHint || '当前选区不支持安全替换，请复制结果。',
      };
    if (!text || text.length > 20000 || text.includes('\0'))
      return { ok: false, message: '替换内容为空、过长或含有不支持的字符。' };
  ...
  private async request(
    ...
    return new Promise((resolve, reject) => {
      ...
      child.stdin.write(
        JSON.stringify({ id, op, deadline: Date.now() + 5500, ...fields }) + '\n',
        'utf8',
        (error) => {
          if (!error) return;
          ...
          if (request) {
            clearTimeout(request.timer);
            this.pending.delete(id);
            request.reject(new Error('无法连接 Windows 选区服务。'));
          }
        },
      );
    });
