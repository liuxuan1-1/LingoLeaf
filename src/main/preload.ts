...
const api: LingoAPI = {
  ...
  analyze: (text, mode, options) => ipcRenderer.invoke('analyze', { text, mode, options }),
  ask: (request) => ipcRenderer.invoke('tutor:ask', request),
  ...
  onResult: (callback) => {
    ...
    let disposed = false;
    void ipcRenderer.invoke('result:get').then((result) => {
      if (result && !disposed) callback(result);
    });
    return () => {
      disposed = true;
      ipcRenderer.removeListener('result', listener);
    };
  },
  onChanged: (callback) => {
    ...
    return () => {
      ipcRenderer.removeListener('changed', listener);
    };
  },
};
