...
export const DEFAULT_SETTINGS: Settings = {
  provider: 'openai', endpoint: PROVIDER_DEFAULTS.openai.endpoint, model: PROVIDER_DEFAULTS.openai.model,
  apiKey: '', hasApiKey: false, azureApiVersion: '2024-10-21',
  targetLanguage: 'English', explanationLanguage: '简体中文',
  grammarShortcut: 'CommandOrControl+Shift+G', translateShortcut: 'CommandOrControl+Shift+T',
...
const settingsSchema = z.object({
  ...
  endpoint: z.string().max(2000), model: z.string().max(200), apiKey: z.string().max(16000), hasApiKey: z.boolean(),
  azureApiVersion: z.string().max(100), targetLanguage: z.string().trim().min(1).max(100),
  explanationLanguage: z.string().trim().min(1).max(100), grammarShortcut: z.string().min(1).max(100),
  translateShortcut: z.string().min(1).max(100), libraryPath: z.string().max(4000).refine(value => path.isAbsolute(value)),
  autoReplace: z.boolean(), launchAtLogin: z.boolean(), saveCorrectSentences: z.boolean(), clearApiKey: z.boolean().optional(),
}).strict();

const reviewSchema = z.object({
  dueAt: z.string().datetime(), interval: z.number().finite().min(0).max(3650), ease: z.number().finite().min(1.3).max(3.5),
  repetitions: z.number().int().min(0).max(1000000), lastReviewedAt: z.string().datetime().nullable(),
}).strict();
const entrySchema = analysisSchema.extend({
  id: z.string().uuid(), createdAt: z.string().datetime(), updatedAt: z.string().datetime(), review: reviewSchema,
  sourceLanguage: z.string().max(100), targetLanguage: z.string().max(100),
}).strict();
const librarySchema = z.object({ version: z.literal(1), entries: z.array(entrySchema).max(100000) }).strict();
const configSchema = z.object({ version: z.literal(1), settings: settingsSchema, credentials: z.record(z.string()) }).strict();

export function credentialScope(settings: Pick<Settings, 'provider' | 'endpoint'>): string {
  const endpoint = (settings.endpoint.trim() || PROVIDER_DEFAULTS[settings.provider].endpoint).replace(/\/+$/, '');
...
function entryFilename(entry: Entry): string { return `${entry.createdAt.slice(0, 10)}-${entry.id}.md`; }

export function renderEntryMarkdown(entry: Entry): string {
  const issueText = entry.issues.length ? entry.issues.map((issue, index) =>
  ...
  return `# ${entry.mode === 'grammar' ? '语法学习' : '翻译学习'} · ${entry.createdAt.slice(0, 10)}\n\n` +
...
function renderIndex(entries: Entry[]): string {
  const now = Date.now();
  const due = entries.filter(entry => Date.parse(entry.review.dueAt) <= now).sort((a, b) => a.review.dueAt.localeCompare(b.review.dueAt));
  const links = (items: Entry[]) => items.map(entry =>
...
export class LibraryStore {
  ...
  constructor(private readonly baseDir: string, private readonly protect: (value: string) => string, private readonly unprotect: (value: string) => string) {
  ...
  async init(): Promise<void> {
    ...
    if (config !== null) {
      ...
      if (!parsed.success) throw new Error('设置文件格式无效。原文件已保留，请从备份恢复 settings.json。');
      const { clearApiKey: _clear, ...saved } = parsed.data.settings;
      this.settings = { ...saved, apiKey: '', hasApiKey: false };
      try {
        this.credentials = Object.fromEntries(Object.entries(parsed.data.credentials).map(([key, value]) => [key, this.unprotect(value)]));
      } catch { throw new Error('无法解密已保存的 API Key。请在原 Windows 用户账户中打开，或备份后重新配置密钥。'); }
    }
  ...
  getSettings(): Settings {
    return { ...this.settings, apiKey: '', hasApiKey: Boolean(this.credentials[credentialScope(this.settings)]) };
  }
  getProviderSettings(): Settings {
    return { ...this.getSettings(), apiKey: this.credentials[credentialScope(this.settings)] || '' };
  }
  getLibraryDirectory(): string { return path.join(this.settings.libraryPath, 'LingoLeaf'); }
  getSyncError(): string | null { return this.syncError; }
  list(): Entry[] { return structuredClone([...this.entries].sort((a, b) => b.createdAt.localeCompare(a.createdAt))); }
  ...
  async saveSettings(value: Settings): Promise<Settings> {
    return this.serial(async () => {
      ...
      if (settings.grammarShortcut.toLowerCase() === settings.translateShortcut.toLowerCase()) throw new Error('语法和翻译快捷键不能相同。');
      ...
      if (settings.libraryPath !== this.settings.libraryPath) await this.writeMirror(publicSettings);
  ...
  async add(analysis: Analysis, settings: Settings): Promise<Entry> {
    return this.serial(async () => {
      ...
      const entry: Entry = {
        ...validated.data, id: randomUUID(), createdAt: timestamp.toISOString(), updatedAt: timestamp.toISOString(),
        review: initialReview(timestamp), sourceLanguage: analysis.mode === 'grammar' ? 'English' : 'Auto',
  ...
  async rate(id: string, rating: Rating): Promise<Entry> {
    return this.serial(async () => {
      ...
      const timestamp = new Date();
      const updated = { ...entry, updatedAt: timestamp.toISOString(), review: scheduleReview(entry.review, rating, timestamp) };
  ...
  async remove(id: string): Promise<void> {
    return this.serial(async () => {
      ...
      this.entries = entries;
      try {
        await fs.rm(path.join(this.getLibraryDirectory(), 'entries', entryFilename(entry)), { force: true });
        await this.refreshMirror();
      } catch { this.syncError = '条目已从应用删除，但 Markdown 文件尚未删除。请检查学习目录权限。'; }
    });
  }
  ...
  private async persistSettings(settings: Settings, credentials: Record<string, string>): Promise<void> {
    const encrypted = Object.fromEntries(Object.entries(credentials).map(([key, value]) => [key, this.protect(value)]));
    await atomicWrite(path.join(this.baseDir, 'settings.json'), JSON.stringify({ version: 1, settings, credentials: encrypted }, null, 2));
  }
  private async persistEntries(entries: Entry[]): Promise<void> {
    await atomicWrite(path.join(this.baseDir, 'library.json'), JSON.stringify({ version: 1, entries }, null, 2));
  }
  private async refreshMirror(): Promise<void> {
    try { await this.writeMirror(this.settings); this.syncError = null; }
    catch { this.syncError = '学习内容已保存在此电脑，但 Markdown 同步失败。请检查学习目录是否在线、可写。'; }
  }
