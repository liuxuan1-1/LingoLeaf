...
const analysis: Analysis = { mode: 'grammar', original: 'She go to school.', corrected: 'She goes to school.', isCorrect: false, explanation: '主语为第三人称单数。', issues: [{ original: 'go', replacement: 'goes', kind: 'grammar', rule: '主谓一致', explanation: '使用 goes。' }], tags: ['主谓一致'], example: 'He walks to work.' };

beforeEach(async () => { directory = await fs.mkdtemp(path.join(os.tmpdir(), 'lingoleaf-store-')); });
...
describe('learning storage', () => {
  it('persists analyses, encrypted credentials, and redacted public settings across a restart', async () => {
    ...
    expect(await fs.readFile(path.join(directory, 'settings.json'), 'utf8')).not.toContain('sensitive-api-key');
    ...
    const note = await fs.readFile(path.join(restarted.getLibraryDirectory(), 'entries', files[0]), 'utf8');
  ...
  it('serializes parallel app/mobile updates without losing cards or review ratings', async () => {
    const store = makeStore(); await store.init();
    const entries = await Promise.all(Array.from({ length: 12 }, (_, index) => store.add({ ...analysis, original: `Card ${index}` }, store.getSettings())));
    expect(store.list()).toHaveLength(12);
    await Promise.all([store.rate(entries[0].id, 'good'), store.rate(entries[0].id, 'good'), store.rate(entries[0].id, 'good')]);
    const restarted = makeStore(); await restarted.init();
    expect(restarted.list()).toHaveLength(12);
    expect(restarted.list().find(entry => entry.id === entries[0].id)?.review).toMatchObject({ repetitions: 3, interval: 8 });
  });
  ...
  it('keeps user files when choosing a sync folder and mirrors existing entries there', async () => {
    const store = makeStore(); await store.init();
    const entry = await store.add(analysis, store.getSettings());
    const sync = path.join(directory, 'sync'); await fs.mkdir(sync);
  ...
  it('never publishes a partial immutable note and retries failed publication from committed data', async () => {
    const store = makeStore(); await store.init();
    vi.spyOn(fs, 'link').mockRejectedValueOnce(Object.assign(new Error('disk failure'), { code: 'EIO' }));
    const entry = await store.add(analysis, store.getSettings());
  ...
  it('returns copies so renderer/mobile callers cannot silently mutate persisted state', async () => {
    ...
    const list = store.list(); list[0].issues[0].rule = 'changed';
  ...
  it('renders hostile Markdown/HTML as literal teaching content without escaping its code fence', async () => {
    const store = makeStore(); await store.init();
    const entry = await store.add({ ...analysis, original: '```\n<script>alert(1)</script>\n[link](evil)', explanation: '<script>alert(1)</script> [click](javascript:evil)', tags: ['[evil](javascript:evil)'] }, store.getSettings());
