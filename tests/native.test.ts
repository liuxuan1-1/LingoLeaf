...
import { NativeService, parseNativeCapture, parseReplacementResult, type NativeCapture } from '../src/main/native';

const capture: NativeCapture = {
  id: 'a'.repeat(32), text: 'I has a question.', appName: 'notepad', processId: 12345, method: 'uia',
  canReplace: true, capturedAt: '2026-09-13T00:00:00.000Z', replacementHint: '',
};

describe('native process boundary guards', () => {
  it('preserves multilingual selected text and an opaque capture identifier', () => {
    expect(parseNativeCapture({ ...capture, text: '今天天气怎么样？ “Hello” 🚀' }).text).toBe('今天天气怎么样？ “Hello” 🚀');
  });

  it('rejects clipboard captures that claim replacement is safe', () => {
    expect(() => parseNativeCapture({ ...capture, method: 'clipboard', canReplace: true })).toThrow();
    expect(parseNativeCapture({ ...capture, method: 'clipboard', canReplace: false }).canReplace).toBe(false);
  });

  it.each([
    null, {}, { ...capture, id: 'not-a-native-id' }, { ...capture, text: ' ' },
    { ...capture, text: 'a'.repeat(20001) }, { ...capture, capturedAt: 'invalid' },
    { ...capture, canReplace: 'true' }, { ...capture, method: 'unknown' },
    { ...capture, processId: undefined }, { ...capture, processId: '12345' },
    { ...capture, processId: 0 }, { ...capture, processId: -1 },
    { ...capture, processId: 1.5 }, { ...capture, processId: Number.NaN },
  ...
  it('does not convert a transport acknowledgement into replacement success', () => {
    ...
    expect(parseReplacementResult({ ok: false, message: '选区已变化' })).toEqual({ ok: false, message: '选区已变化' });
  });
  ...
  it('refuses unreplaceable selection and invalid output before starting a process', async () => {
    ...
    expect((await service.replace({ ...capture, canReplace: false, replacementHint: '请复制结果' }, 'Hello')).ok).toBe(false);
  ...
  it.runIf(process.platform === 'win32')('starts the real hidden helper and rejects an unknown capture without any UI action', async () => {
