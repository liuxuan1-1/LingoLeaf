...
| 模式 | 输入 | 获得的内容 |
| --- | --- | --- |
| 语法纠错 | 英语句子 | 最少改动的纠错、原因，以及讲解语言的完整译文 |
| 翻译表达 | 任意语言的句子 | 翻译成设置中的目标语言 |
| 阅读解析 | 目标语言的文本，例如英文 | 翻译成讲解语言，按原文片段解析语法结构，并总结学习要点 |
| 帮我表达 | 零散想法、关键词、中英混合描述；可补充场景和语气 | 目标语言的推荐表达、两到三个备选及语气区别；意思不确定时给出澄清问题 |

需要英译中时，把「目标语言」设为 **English**、「讲解语言」设为 **简体中文**，在练习页选择「阅读解析」。原有全局快捷键继续用于纠错和正向翻译。
...
当前版本：**v0.3.0 预览版**。从 [Releases](https://github.com/liuxuan1-1/LingoLeaf/releases) 下载 Windows x64 免安装程序，启动后先配置自己的模型接口。
